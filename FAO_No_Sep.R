...
#reshape into long format
BAS_long <- BAS_data %>%
  pivot_longer(cols = 6:ncol(BAS_data), #metabolite columns only
               names_to = "Metabolite",
               values_to = "Concentration")

#loop through each patient and metabolite
patients <- unique(BAS_long$Patient)

#open a PDF device
pdf("/Users/marcinebessire/Desktop/Master_Thesis/No_Separation/BAS_original_Kinetics.pdf", width = 14, height = 10)
